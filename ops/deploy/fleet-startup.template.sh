...
# The direct service URL: coder.openagents.com now names Rust Coder, and the
# inference hop must keep reaching the openagents-coder-api service itself.
export OPENAGENTS_CODER_API_ORIGIN="https://openagents-coder-api-157437760789.us-central1.run.app"
export OPENAGENTS_CODER_API_INTERNAL_TOKEN="$(secret openagents-coder-api-internal-token)"
# Signs the Coder-audience tokens /api/v1/coder/token mints (A2).
export CODER_TOKEN_SIGNING_KEY="$(secret coder-token-signing-key)"
...
ENV_NAMES=(
  ...
  OPENAGENTS_INFERENCE_PROXY_URL OPENAGENTS_CODER_API_ORIGIN
  OPENAGENTS_CODER_API_INTERNAL_TOKEN CODER_TOKEN_SIGNING_KEY
  OPENAGENTS_MACHINE_TOKEN_TTL_SECONDS
